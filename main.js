...
(function () {
    function detect() {
        ...
        for (var i = 0; i < forceDomains.length; i++) {
            if (('.' + location.host).substr(-forceDomains[i].length - 1) == '.' + forceDomains[i]) {
                return 'domain';
            }
        }
        ...
        var l, ls = [];
        if (document.querySelector('meta[http-equiv="Content-Language" i]')) {
            l = document.querySelector('meta[http-equiv="Content-Language" i]').getAttribute('content');
            if (l) {
                ls.push(l.toLowerCase());
            }
        }
        if (document.querySelector('meta[property="og:locale" i]')) {
            l = document.querySelector('meta[property="og:locale" i]').getAttribute('content');
            if (l) {
                ls.push(l.toLowerCase());
            }
        }
        ...
        for (var i = 0; i < ls.length; i++) {
            if (llst.indexOf(ls[i].substr(0, 2)) != -1 || /[-_](cn|sg|my|hans)/.test(ls[i])) {
                return '';
            }
        }
        ...
        if (/[網頁國電灣歡臺為華資訊體麼類學詢種級閱會統員總談時話]/.test(desc)) {
            return  'meta content';
        }
        var css = '';
        ...
        if (/標|體|軟|繁|驛|儷|蘋|JhengHei|正黑|CJK TC|PingFang TC/i.test(css)) {
            return 'css';
        }
        if (window.parent != window.self) {
            ...
            walker = null;
            if (/[應檢舉鐘評論網頁號關開訪註幫會圖聯員讚歡話]/.test(s)) {
                return 'frame text';
            }
        }
        return '';
